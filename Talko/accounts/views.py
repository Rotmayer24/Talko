...
# How long a verification link stays valid, and how long to wait between resends.
...
def verify_email(request, token):
    # Look the link up by token so it works straight from the email, regardless
    # of whether anyone is logged in (the user isn't after registration).
    profile = UserProfile.objects.filter(email_verification_token=token).first() if token else None
    if profile is None:
