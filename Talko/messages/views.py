...
# Extensions that can be interpreted/executed by the browser or OS and must
# never be accepted, since media is served from the same origin.
BLOCKED_FILE_EXTENSIONS = {
    ".html", ".htm", ".svg", ".xml", ".js", ".mjs", ".php", ".phtml",
    ".exe", ".bat", ".cmd", ".com", ".sh", ".msi", ".jar", ".py", ".pl",
    ".rb", ".dll", ".scr", ".vbs", ".ps1",
}
...
def upload_file(request, chat_id):
    ...
    if image:
        ...
        if ext in BLOCKED_FILE_EXTENSIONS:
            return JsonResponse({'error': 'This file type is not allowed'}, status=400)
...
def delete_message(request, chat_id, message_id):
    chat = get_object_or_404(Chat, id=chat_id, users=request.user)
    message = get_object_or_404(Message, id=message_id, chat=chat, sender=request.user)
